...
class PhaseCameraNode(Node):
    ...
    def __init__(self):
        ...
        parser.add_argument('--input', type=str, default="40098270", help="Input folder of images")
        args, unknown = parser.parse_known_args()
        
        self.input_folder_ = args.input
        ...
        self.filenames = [img for img in glob.glob(self.input_folder_+"*_l.png")]
    ...
    def read_frame(self):
        try:
            img = self.filenames[self.count_]
        except:
            img = 0
        if img:
            read_result_left = cv2.imread(img.split('_')[0]+"_l.png")
            read_result_right = cv2.imread(img.split('_')[0]+"_r.png")
            # self.get_logger().info("Stereo result received")
            rect_image_pair = self.calibration_.rectify(read_result_left, read_result_right)
            ...
            self.publish_image(self.pub_img_rawleft_, read_result_left, header = header)
            self.publish_image(self.pub_img_rawright_, read_result_right, header = header)
